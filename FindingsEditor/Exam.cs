...
namespace FindingsEditor
{
    public class Exam
    {
        ...
        private void readExamData(int _exam_id)
        {
            ...
            {
                conn.Close();
                MessageBox.Show(Properties.Resources.NoRecord, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            else
            {
        ...
        public void saveFindingsEtc()
        {
            ...
            feFunctions.ExeNonQuery(sql, purpose, order_dr, pathology.ToString(), patho_no, reply_patho.ToString(),
            ...
            if (department != 0)
            {
                ...
                switch (feFunctions.ExeNonQuery(sql, department.ToString()))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
                    case feFunctions.functionResult.failed:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.DataBaseError, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            if (!string.IsNullOrWhiteSpace(ward_id))
            {
                column = "ward_id";
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, ward_id))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            {
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, operator1))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            {
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, operator2))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            {
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, operator3))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            {
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, operator4))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            {
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, operator5))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            if (!string.IsNullOrWhiteSpace(diag_dr))
            {
                column = "diag_dr";
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, diag_dr))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            if (!string.IsNullOrWhiteSpace(final_diag_dr))
            {
                ...
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, final_diag_dr))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            if (equipment != 0)
            {
                column = "equipment";
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, equipment.ToString()))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
            ...
            if (place_no != 0)
            {
                column = "place_no";
                sql = "UPDATE exam SET " + column + "=:p0 WHERE exam_id=" + exam_id.ToString();
                switch (feFunctions.ExeNonQuery(sql, place_no.ToString()))
                {
                    case feFunctions.functionResult.connectionError:
                        MessageBox.Show("[" + column + "]" + Properties.Resources.ConnectionFailed, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        break;
        ...
        public string getExamTypeName()
        {
            ...
            return feFunctions.getSelectString(sql, Settings.DBSrvIP, Settings.DBSrvPort, Settings.DBconnectID, Settings.DBconnectPw, Settings.DBname);
        }

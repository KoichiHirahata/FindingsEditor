【ソフト名】Findings Editor（内視鏡・超音波検査所見入力システム） Ver.1.25.8
...
・動作に必要なソフトウェア
.NET Framework 4.5.2以降が必要です。

.NET FrameworkはWindows updateでインストールが可能です。

インターネットに接続していないPCでFindings Editor（以下、本ソフトウェア）が
動作しないようであれば、
https://www.microsoft.com/ja-jp/download/details.aspx?id=53344
にアクセスし、NDP462-KB3151800-x86-x64-AllOS-ENU.exeをダウンロードして
インストールしてください。
...
・履歴
2020/05/23 Ver. 1.25.9　患者情報更新画面のバグ修正
2019/02/26 Ver. 1.25.8　バグ修正、施行者リストでの科名表示修正。
2018/06/06 Ver. 1.25.6　検査登録、検査記録、施行者登録、病名登録のバグ修正等。
2017/07/23 Ver. 1.25.3　所見保存時に空欄があると保存できないバグ修正。
2017/07/19 Ver. 1.25.2　初期設定フォームのバグ修正。
2017/07/17 Ver. 1.25.1　病名が登録されていないタイプの検査でのバグ修正。
　　　　　　　　　　　　同梱の患者画像管理のバージョンアップ。
